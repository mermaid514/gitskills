...
class DetectionValidator(BaseValidator):
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            if nl:
                ...
                labelsn = torch.cat((cls, color, tbox), 1)  # native-space labels
    ...
    def _process_batch(self, detections, labels):
        ...
        Arguments:
            detections (array[N, 7]), x1, y1, x2, y2, conf, class,color
            labels (array[M, 6]), class, color, x1, y1, x2, y2
        Returns:
        ...
        correct_color = labels[:,1:2] == detections[:, 6]
        correct_cls_color = correct_color & correct_class
        for i in range(len(self.iouv)):
            x = torch.where((iou >= self.iouv[i]) & correct_cls_color)  # IoU > threshold and classes & color match
            if x[0].shape[0]:
