...
def non_max_suppression(
    ...
    xcolor = prediction[:,mi:].amax(1) > conf_thres # candidates
    # Settings
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box, cls, color = x.split((4, nc, ncolor), 1)
        box = xywh2xyxy(box)  # center_x, center_y, width, height) to (x1, y1, x2, y2)
        if multi_label:
            i, j = (cls > conf_thres).nonzero(as_tuple=False).T
            color_value,color_index = color[i].max(-1,keepdim=True)
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), color_index.float()), 1)
