...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ncolor=12,ch=()):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.ncolor = ncolor #number of colors
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  + self.ncolor # number of outputs per anchor
        ...
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3,self.ncolor,1)) for x in ch)
    ...
    def forward(self, x):
        ...
        box, cls, color = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2).split((self.reg_max * 4, self.nc, self.ncolor), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = torch.cat((dbox, cls.sigmoid(), color.sigmoid()), 1)
    ...
    def bias_init(self):
        ...
        for a, b,c, s in zip(m.cv2, m.cv3, m.cv4, m.stride):  # from
            ...
            c[-1].bias.data[:m.nc] = math.log(5 / m.ncolor / (640 / s) ** 2)
